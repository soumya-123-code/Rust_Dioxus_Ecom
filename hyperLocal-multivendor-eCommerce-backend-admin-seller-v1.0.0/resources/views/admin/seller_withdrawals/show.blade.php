@extends('layouts.admin.app', ['page' => $menuAdmin['seller_management']['active'] ?? "", 'sub_page' => $menuAdmin['seller_management']['route']['seller_withdrawals']['sub_active']])
                    ...
                    <div class="btn-list">
                        @if($withdrawalRequest->status === 'pending' && $processRequestPermission ?? false)
                            <button type="button" class="btn btn-primary process-withdrawal-request"
                                    ...
                                    data-seller-id="{{ $withdrawalRequest->seller_id }}"
                                    data-amount="{{ $withdrawalRequest->amount }}">
                                {{ __('labels.process_request') }}
                            </button>
                        @endif
                        <a href="{{ route('admin.seller-withdrawals.index') }}" class="btn btn-outline-primary">
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table mb-0" style="border: none;">
                                    ...
                                    <tr>
                                        <td class="fw-bold border-0">{{ __('labels.seller') }}</td>
                                        <td class="border-0">{{ $withdrawalRequest->seller->user->name ?? 'N/A' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="fw-bold border-0">{{ __('labels.amount') }}</td>
                                        <td class="border-0">{{ $systemSettings['currencySymbol'] . number_format($withdrawalRequest->amount, 2) }}</td>
                                    </tr>
                                    <tr>
        ...
        <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body text-center py-4">
                    ...
                    <div class="mt-3">
                        <div class="text-muted">{{ __('labels.seller') }}: <span id="withdrawal-seller">{{ $withdrawalRequest->seller->user->name ?? 'N/A' }}</span></div>
...
@push('scripts')
    <script src="{{asset('assets/js/seller-withdrawals.js')}}" defer></script>
@endpush
