...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> _onLoginRequest(
    ...
    try {
      ...
      if (response.first.success == true) {
        ...
        emit(AuthSuccess(message: response.first.message ?? 'Login successful'));
      } else {
  ...
  Future<void> _onRegisterRequest(
    ...
    try {
      ...
      if (response.first.success == true) {
        ...
        emit(AuthSuccess(message: response.first.message ?? 'Register successful'));
      } else {
        String errorMessage = response.first.message ?? 'Register failed';

        if (errorMessage.toLowerCase().contains('mobile has already been taken')) {
          errorMessage =
  ...
  Future<void> _onSocialAuthRequest(
    ...
    try {
      final response =
      await _repository.socialAuth(
        firebaseToken: event.firebaseToken,
        isApple: event.isApple
      );
      if (response['success'] == false && response['data']['new_user'] == true) {
        emit(SocialAuthSuccess(
          newUser: response['data']['new_user'] != null ? response['data']['new_user'] ?? true : true,
          userName: response['data']['name'] != null ? response['data']['name'] ?? '' : '',
          userEmail: response['data']['email'] != null ? response['data']['email'] ?? '' : '',
        ));
        ...
        emit(AuthSuccess(message: userData.first.message ?? 'Login successful'));
      }
    } catch (e) {
      emit(AuthFailed(error: e.toString()));
    }
